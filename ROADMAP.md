...
- [ ] **Card appearance option — Default vs. HASS UI** `high · med` — **top priority.** Add a card-level config key `appearance: default | hass_ui` on both `hass-console-card` and `hass-console-summary-card`:
  - `default` (unchanged, the current Niagara-inspired look) — existing dashboards render exactly as they do today; no visual regression for anyone who has already themed their console.
  - `hass_ui` — restyle the cards so they read as native Home Assistant surfaces. Drive colors from HA's own CSS variables (`--ha-card-background`, `--card-background-color`, `--primary-text-color`, `--secondary-text-color`, `--divider-color`, `--primary-color`, `--state-icon-color`) instead of the hard-coded Niagara palette. Remap the severity chips/gauges to HA's state colors (`--error-color` → Critical, `--warning-color` → Major, `--info-color` → Minor, `--success-color` → cleared/ACK'd) so they color-code the same way HA badges, alerts, and entity states do out of the box. Match Ha-Card padding, corner radius, and typography so the console sits naturally next to Weather / Media / Entities cards on the same dashboard.
  - Distinct from the existing `theme: auto | dark | light` (which controls dark/light mode following). `appearance` controls *visual style*; `theme` controls *color mode*. The two combine — you can run `appearance: hass_ui, theme: auto` and get HA-native styling that also follows the user's light/dark preference.
  - Wire the option into the visual card editor once that lands (see **Visual card editors for both cards** below).
  - Ship both cards' `hass_ui` styling in the same release so users don't see the main card go native while the summary card stays Niagara.
